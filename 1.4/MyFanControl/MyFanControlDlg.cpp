...
void CMyFanControlDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_GPU_POWERSAVE, m_ctlGPUPowerSave);
	DDX_Control(pDX, IDC_BUTTON_GPU_POWERSAVE, m_ctlGPUPowerSaveSettings);
}
...
BEGIN_MESSAGE_MAP(CMyFanControlDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHECK_GPU_POWERSAVE, &CMyFanControlDlg::OnBnClickedCheckGpuPowerSave)
	ON_BN_CLICKED(IDC_BUTTON_GPU_POWERSAVE, &CMyFanControlDlg::OnBnClickedButtonGpuPowerSave)
...
BOOL CMyFanControlDlg::OnInitDialog()
{
	...
	SetTimer(0, 100, NULL);
	// 根据配置初始化开机自启复选框状态
	m_ctlAutorun.SetCheck(m_core.m_config.AutoRun);
...
void CMyFanControlDlg::OnBnClickedCheckGpuPowerSave()
{
	int val = m_ctlGPUPowerSave.GetCheck();
	m_core.m_config.EnablePowerSaving = val;
}

void CMyFanControlDlg::OnBnClickedButtonGpuPowerSave()
{
	CGPUPowerSaveDlg dlg(this, m_core.m_GpuInfo);
	dlg.m_nBatteryFreq = m_core.m_config.BatteryGPUFrequency;
	dlg.m_nBatteryMem = m_core.m_config.BatteryMemOffset;
	dlg.m_nPowerFreq = m_core.m_config.PowerGPUFrequency;
	dlg.m_nPowerMem = m_core.m_config.PowerMemOffset;

	if (dlg.DoModal() == IDOK)
	{
		m_core.m_config.BatteryGPUFrequency = dlg.m_nBatteryFreq;
		m_core.m_config.BatteryMemOffset = dlg.m_nBatteryMem;
		m_core.m_config.PowerGPUFrequency = dlg.m_nPowerFreq;
		m_core.m_config.PowerMemOffset = dlg.m_nPowerMem;
		m_core.m_config.SaveConfig();
		// 若节电模式当前启用，则立即生效（下一次 Work 循环）
	}
}
...
void CMyFanControlDlg::UpdateGui(BOOL bFull)
{
	...
	// 更新 GPU 节电复选框状态
	int gps = m_ctlGPUPowerSave.GetCheck();
	if (gps ^ m_core.m_config.EnablePowerSaving)
	{
		m_ctlGPUPowerSave.SetCheck(m_core.m_config.EnablePowerSaving);
	}
...
BOOL CMyFanControlDlg::CheckAndSave()
{
	...
	// 获取 GPU 节电模式状态
	BOOL bEnablePowerSaving = m_ctlGPUPowerSave.GetCheck();
	...
	m_core.m_config.EnablePowerSaving = bEnablePowerSaving;
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j < 10; j++)
		{
...
void CMyFanControlDlg::SetAdvancedMode(BOOL bAdvanced)
{
	...
	{
		MoveWindow(rect.left, rect.top, m_nWindowSize[0] * 335 / 582, m_nWindowSize[1] * 283 / 510, FALSE);
...
// 开机自启：纯计划任务方式，不再弹出选择框
void CMyFanControlDlg::OnBnClickedCheckAutorun()
{
	int val = m_ctlAutorun.GetCheck();
	BOOL result = SetAutorunTask(TRUE, val ? TRUE : FALSE);
	if (!result)
	{
		// 操作失败，恢复复选框状态
		m_ctlAutorun.SetCheck(!val);
	}
	else
	{
		m_core.m_config.AutoRun = val;
		m_core.m_config.SaveConfig();
	}
}

// 注册表自启函数已废弃，保留以备兼容，但不再被调用
...
BOOL CMyFanControlDlg::SetAutorunTask(BOOL bWrite, BOOL bAutorun)
{
	...
	if (rs.Find(strFind) >= 0)
	{
		// 同步配置状态
		if (bWrite)
			m_core.m_config.AutoRun = bAutorun;
		m_core.m_config.SaveConfig();
		return TRUE;
	}
	return FALSE;
...
BOOL CMyFanControlDlg::CreateTaskXml(PCSTR strXmlPath, PCSTR strTargetPath)
{
	// 任务计划配置：
	// - 用户登录时触发
	// - 最高可用权限 (HighestAvailable)
	// - 允许按需执行 (AllowStartOnDemand)
	// - 如果过了计划开始时间立即执行 (StartWhenAvailable)
	// - 如果已有实例运行则强行终止 (MultipleInstancesPolicy=StopExisting)
	PCSTR XmlStr = "\
