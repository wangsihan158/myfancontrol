...
public:
	...
	BOOL SoftControl;//软性控制开关
	//GPU控制
	BOOL LockGPUFrequency;//是否控制GPU频率
	int GPUFrequency;//GPU频率限制
	BOOL LockMemOverclock;//是否锁定显存偏移
	int MemOverclockOffset;//显存频率偏移
	BOOL AutoRun;   // 是否开启计划任务自启

	// GPU 节电模式
	BOOL EnablePowerSaving;          // 是否启用节电模式
	int BatteryGPUFrequency;         // 电池模式 GPU 核心频率
	int BatteryMemOffset;            // 电池模式显存偏移
	int PowerGPUFrequency;           // 电源模式 GPU 核心频率
	int PowerMemOffset;              // 电源模式显存偏移

	// 可调温度档位
	int TempThresholds[10];//10个温度档位，默认{90,85,80,75,70,65,60,55,50,45}
...
public:
	...
	// 软性控制相关
	int m_nSoftTargetDuty[2];//软性控制目标负载
	int m_nSoftCurrentDuty[2];//软性控制当前实际负载
...
public:
	...
	void Uninit();
	void Run();//内核主循环
	void RunOriginal();//原始运行方式（备用）
	void Work();//更新温度、负载，计算负载并进行设置
	void Update();//更新风扇状态
	void Control();//控制
	void CalcLinearDuty();//求得线性平滑后的负载
	void CalcStdDuty();//求得当前温度对应的负载
	void ResetFan();//重置风扇为自动控制
	void SetFanDuty();//设置风扇转速
	void SoftControlDuty();//软性控制风扇转速
};