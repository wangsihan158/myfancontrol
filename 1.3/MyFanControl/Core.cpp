...
void CConfig::LoadConfig()
{
	...
	if (fileLen != sizeof(CConfig))
	{
		LoadDefault();
		if (fileLen > sizeof(CConfig))
			fileLen = sizeof(CConfig);  // 截断
		file.Read(this, (UINT)fileLen);
	}
	else
	{
		file.Read(this, sizeof(CConfig));
	}
	file.Close();
}
...
// 检测当前为电池供电还是交流供电
BOOL CCore::IsOnBattery()
{
	SYSTEM_POWER_STATUS sps;
	if (GetSystemPowerStatus(&sps))
		return (sps.ACLineStatus == 0);  // 0=电池, 1=交流, 255=未知
	return FALSE;  // 未知保守当作插电
}
...
void CCore::Work()
{
	...
	// GPU 节电模式优先
	if (m_config.EnablePowerSaving)
	{
		// 节电模式启用时，根据电源状态使用节电设置
		BOOL bOnBattery = IsOnBattery();
		int targetFreq = bOnBattery ? m_config.BatteryGPUFrequency : m_config.PowerGPUFrequency;
		int targetMem = bOnBattery ? m_config.BatteryMemOffset : m_config.PowerMemOffset;

		// 应用 GPU 核心频率
		if (targetFreq != 0)
			m_GpuInfo.LockFrequency(targetFreq);
		else
			m_GpuInfo.LockFrequency(0);  // 0 释放锁定，使用默认频率

		// 应用显存偏移
		m_GpuInfo.SetMemOverclockOffset(targetMem);
	}
	else
	{
		// 节电模式关闭时，使用原有的手动设置
